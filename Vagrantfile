...
Vagrant.configure("2") do |config|
	...
	config.vm.provision "shell", inline: <<-SHELL
		...
		echo "#{server1_private_ip} host1.local.home.lan dnsserverer" >> /etc/hosts
		echo "#{server2_private_ip} host2.local.home.lan" >> /etc/hosts
		echo "#{server3_private_ip} host3.local.home.lan" >> /etc/hosts
	...
	config.vm.define "apatch_httpd-server-balancer" do |server3|
		...
		server3.vm.provision "shell", inline: <<-SHELL
			...
			echo "worker.myworker.host=#{server1_private_ip}" >> /etc/httpd/conf/workers.properties
			...
			echo "worker.other.host=#{server3_private_ip}" >> /etc/httpd/conf/workers.properties
